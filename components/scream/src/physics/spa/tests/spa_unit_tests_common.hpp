...
struct UnitWrap {
  ...
  struct UnitTest : public KokkosTypes<D> {
    ...
    using SPAFunctions       = scream::spa::SPAFunctions<Real, Device>;
    using Scalar             = typename SPAFunctions::Scalar;
    using Spack              = typename SPAFunctions::Spack;
    using Pack               = typename SPAFunctions::Pack;
    using Smask              = typename SPAFunctions::Smask;

    static constexpr Int max_pack_size = 16;
    static constexpr Int num_test_itrs = max_pack_size / Spack::n;

    // Put struct decls here
    struct TestHorizontalInterp;
  };
};
