...
void SurfaceCouplingExporter::set_grids(const std::shared_ptr<const GridsManager> grids_manager)
{
  ...
  add_field<Computed>("Sa_z",       scalar2d_layout, Wm2, grid_name);
  add_field<Computed>("Sa_u",       scalar2d_layout, Wm2, grid_name); 
  add_field<Computed>("Sa_v",       scalar2d_layout, Wm2, grid_name); 
  add_field<Computed>("Sa_tbot",    scalar2d_layout, Wm2, grid_name); 
  add_field<Computed>("Sa_ptem",    scalar2d_layout, Wm2, grid_name);
  add_field<Computed>("Sa_pbot",    scalar2d_layout, Wm2, grid_name); 
  add_field<Computed>("Sa_shum",    scalar2d_layout, Wm2, grid_name); 
  add_field<Computed>("Sa_dens",    scalar2d_layout, Wm2, grid_name);
  add_field<Computed>("Sa_pslv",    scalar2d_layout, Wm2, grid_name);
  add_field<Computed>("Faxa_rainl", scalar2d_layout, Wm2, grid_name);
  add_field<Computed>("Faxa_snowl", scalar2d_layout, Wm2, grid_name);
...
void SurfaceCouplingExporter::compute_eamxx_exports(const double dt, const bool called_during_initialization)
{
  ...
  const auto out_Sa_z       = get_field_out("Sa_z"      ).get_view<Real*>();
  const auto out_Sa_u       = get_field_out("Sa_u"      ).get_view<Real*>(); 
  const auto out_Sa_v       = get_field_out("Sa_v"      ).get_view<Real*>(); 
  const auto out_Sa_tbot    = get_field_out("Sa_tbot"   ).get_view<Real*>(); 
  const auto out_Sa_ptem    = get_field_out("Sa_ptem"   ).get_view<Real*>();
  const auto out_Sa_pbot    = get_field_out("Sa_pbot"   ).get_view<Real*>(); 
  const auto out_Sa_shum    = get_field_out("Sa_shum"   ).get_view<Real*>(); 
  const auto out_Sa_dens    = get_field_out("Sa_dens"   ).get_view<Real*>();
  const auto out_Sa_pslv    = get_field_out("Sa_pslv"   ).get_view<Real*>();
  const auto out_Faxa_rainl = get_field_out("Faxa_rainl").get_view<Real*>();
  const auto out_Faxa_snowl = get_field_out("Faxa_snowl").get_view<Real*>();
  const auto out_Faxa_swndr = get_field_out("Faxa_swndr").get_view<Real*>();
  const auto out_Faxa_swvdr = get_field_out("Faxa_swvdr").get_view<Real*>();
  const auto out_Faxa_swndf = get_field_out("Faxa_swndf").get_view<Real*>();
  const auto out_Faxa_swvdf = get_field_out("Faxa_swvdf").get_view<Real*>();
  const auto out_Faxa_swnet = get_field_out("Faxa_swnet").get_view<Real*>();
  const auto out_Faxa_lwdn  = get_field_out("Faxa_lwdn" ).get_view<Real*>();
