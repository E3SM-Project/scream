                      ...
                      const util::TimeStamp& ts, const bool write_nsteps)
{
  ...
  if (write_nsteps) {
    scorpio::set_global_attribute(filename,ts_name+"_nsteps",ts.get_num_steps());
  }
}
...
{
  auto ts = util::str_to_time_stamp(scorpio::get_global_attribute<std::string>(filename,ts_name));
  if (read_nsteps and scorpio::has_global_attribute(filename,ts_name+"_nsteps")) {
    ts.set_num_steps(scorpio::get_global_attribute<int>(filename,ts_name+"_nsteps"));
  }
  return ts;
}
