...
OutputManager::
OutputManager (const ekat::Comm& io_comm,
               const ekat::ParameterList& params,
               const util::TimeStamp& run_t0,
               const util::TimeStamp& case_t0,
               const bool is_model_restart_output)
  : m_io_comm(io_comm), m_params(params), m_is_model_restart_output(is_model_restart_output)
{
  // Sanity checks
...
void OutputManager::
process_params (const std::map<std::string,std::shared_ptr<fm_type>>& field_mgrs)
{
