...
//   decompositions for the same dim (this feature is already used
//   in the horiz remappers)
...
//    and its vars deleted (from online memory), so a call to this function may be quick
//    (if we can recycle an existing decomp)
...
// Shortcut, to allow calling set_attribute with C strings, like so
