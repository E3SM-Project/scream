...
// =========================================================================================
ZonalWindDiagnostic::ZonalWindDiagnostic (const ekat::Comm& comm, const ekat::ParameterList& params)
...
// =========================================================================================
void ZonalWindDiagnostic::set_grids(const std::shared_ptr<const GridsManager> grids_manager)
{
...
// =========================================================================================
void ZonalWindDiagnostic::compute_diagnostic_impl()
{
  const auto npacks  = ekat::npack<Pack>(m_num_levs);
  const auto& horiz_winds        = get_field_in("horiz_winds").get_view<const Pack***>();
  const auto& u = m_diagnostic_output.get_view<Pack**>();

  Kokkos::parallel_for("ZonalWindDiagnostic",
                       Kokkos::RangePolicy<>(0,m_num_cols*npacks),
                       KOKKOS_LAMBDA (const int& idx) {
      const int icol  = idx / npacks;
      const int jpack = idx % npacks;
      u(icol,jpack) = horiz_winds(icol,0,jpack);
  });
  Kokkos::fence();
