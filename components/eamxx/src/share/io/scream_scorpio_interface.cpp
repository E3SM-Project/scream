...
int set_dim_decomp (const std::string& filename,
                    const std::string& dimname,
                    const std::string& decomp_name)
{
  const auto& comm = ScorpioSession::instance().comm;
  ...
  set_dim_decomp (filename,dimname,offset,len,decomp_name);
...
void read_var (const std::string &filename, const std::string &varname, T* buf, const int time_index)
{
  ...
  if (get_dtype<T>()!=var.dtype) {
    change_var_dtype (filename, varname, get_dtype<T>());
  }
...
{
  register_file(filename,Read);
  const int ncid = get_file(filename,"scorpio::has_attribute").ncid;
  ...
  if (varname=="GLOBAL") {
    ...
    const auto& var = get_var(filename,varname,"scorpio::has_attribute");
    varid = var.ncid;
  }
                             ...
                             const std::string& attname)
{
  ...
  if (varname=="GLOBAL") {
    ...
    varid = get_var(filename,varname,"scorpio::get_any_attribute").ncid;
  }
                        ...
                        const ekat::any& att)
{
  ...
  if (varname=="GLOBAL") {
    ...
    varid = get_var(filename,varname,"scorpio::set_any_attribute").ncid;
  }
