...
void run(std::mt19937_64& engine)
{
  ...
  auto diag = diag_factory.create("ZonalWind",comm,params);
  diag->set_grids(gm);
...
//      Kokkos::deep_copy(v_sub,std::numeric_limits<Real>::quiet_NaN());
    ...
    Field u_f = diag_out.clone();
    u_f.deep_copy<double,Host>(0.0);
    u_f.sync_to_dev();
    ...
    //  u_v(icol,jpack) = horiz_winds_v(icol,jpack,0);
   ...
   // });
    ...
    Kokkos::fence();
    REQUIRE(views_are_equal(diag_out,u_f));
  }
...
TEST_CASE("zonal_wind_test", "zonal_wind_test]"){
