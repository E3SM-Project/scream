                        ...
                        Smask& log_wetgrowth, Spack& qrcol, Spack& qccol, Spack& qwgrth, Spack& nrshdr, Spack& qcshd)
{
   ...
   constexpr Scalar twopi  = C::Pi*2.0;
   constexpr Scalar half   = C::HALF;
   ...
   const auto qc_qr_incld_ge_small = (qc_incld + qr_incld) >= qsmall;
   const auto qccol_qrcol_ge_small = (qccol + qrcol) >= qsmall;
   ...
   const Spack e0 = polysvp1(zerodeg, 0.0);
   Spack qsat0{0.0};
   Spack dum{0.0};
   Spack dum1{0.0};
   ...
   qwgrth.set(any_if,
             ((f1pr05+f1pr14*pack::cbrt(sc)*pack::pow(rhofaci*rho/mu, half))*
   ...
   dum.set(any_if,
           pack::max(zero, (qccol+qrcol)-qwgrth));

   auto const dum_ge_small = dum >= qsmall;

   nrshdr.set(any_if && dum_ge_small,
              nrshdr+dum*sp(1.923e+6));

   dum1.set(any_if_col && dum_ge_small,
            one/(qccol+qrcol));
