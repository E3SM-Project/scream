...
subroutine dp_inverse_f(nlev, shcol, rho_zt, dz_zt, rdp_zt) bind(C)
  ...
  subroutine isotropic_ts_f(nlev, shcol, brunt_int, lambda_low, lambda_high, lambda_slope, lambda_thresh, tke, a_diss, brunt, isotropy) bind(C)
    use iso_c_binding

    integer(kind=c_int) , value, intent(in) :: nlev, shcol
    real(kind=c_real) , intent(in) :: lambda_low, lambda_high, lambda_slope, lambda_thresh
