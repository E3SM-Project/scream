...
struct CaarFunctorImpl {
  ...
  CaarFunctorImpl(const Elements &elements, const Tracers &/* tracers */,
      ...
      , m_tu(m_policy_pre)
  {
    m_policy_pre = m_policy_pre.set_scratch_size(0, Kokkos::PerTeam(shmem_size(0)));
  ...
  static size_t shmem_size(int /* team_size */) {
    return 2*NP*NP*NUM_LEV_P*sizeof(Scalar) + // To hold a vector field over lev interfaces
           (2*2*2*NP*NP + 2*NP*NP) * sizeof(Real);  // To hold dvv,D,Dinv,metdet
  }
  ...
  template<int NL,typename InputProvider>
  KOKKOS_INLINE_FUNCTION
  static void
  load_shared_scalar (const KernelVariables& kv,
                      const InputProvider& global,
                      ScratchView<Scalar[NP][NP][NL]>& shared)
  {
    auto do_copy = [&](const int idx) {
      int i   = (idx / NL) / NP;
      int j   = (idx / NL) % NP;
      int k   =  idx % NL;
      shared(i,j,k) = global(i,j,k);
    };
    Kokkos::parallel_for(Kokkos::TeamVectorRange(kv.team,NP*NP*NL),do_copy);
  }

  template<int NL,typename InputProvider>
  KOKKOS_INLINE_FUNCTION
  static ScratchView<Scalar[NP][NP][NL]>
  load_shared_scalar (const KernelVariables& kv,
                      const InputProvider& global)
  {
    ScratchView<Scalar[NP][NP][NL]> shared(reinterpret_cast<Scalar*>(kv.team_shmem));
    load_shared_scalar(kv,global,shared);
    return shared;
  }
  ...
  void operator()(const TagPreExchange&, const TeamMember &team, int& nerr) const {
    ...
    kv.init_team_shmem( (2*NP*NP*2*2 + 2*NP*NP)*sizeof(Real), // We will need D (or Dinv), metdet, and dvv
                       shmem_size(team.team_size()));

    // Put dvv and geo views into shared memory
    auto geo_data_shmem = kv.geo_data_shmem;
    kv.dvv = decltype(kv.dvv)(geo_data_shmem); geo_data_shmem += NP*NP;
    kv.metdet = decltype(kv.metdet)(geo_data_shmem); geo_data_shmem += NP*NP;
    kv.Dinv = decltype(kv.Dinv)(geo_data_shmem); geo_data_shmem += NP*NP*2*2;
    kv.D = decltype(kv.D)(geo_data_shmem); geo_data_shmem += NP*NP*2*2;

    Kokkos::parallel_for(Kokkos::TeamVectorRange(kv.team,NP*NP),
                         [&](int idx) {
      const int i = idx / NP;
      const int j = idx % NP;
      kv.dvv(i,j) = m_deriv(i,j);
      kv.metdet(i,j) = m_geometry.m_metdet(kv.ie,i,j);
    });
    Kokkos::parallel_for(Kokkos::TeamVectorRange(kv.team,NP*NP),
                         [&](int idx) {
      const int idim = ((idx % NP) / NP) / 2;
      const int jdim = ((idx % NP) / NP) % 2;
      const int ipt  =  (idx / NP) % NP;
      const int jpt  =   idx % NP;
      kv.Dinv(idim,jdim,ipt,jpt) = m_geometry.m_dinv(kv.ie,idim,jdim,ipt,jpt);
      kv.D   (idim,jdim,ipt,jpt) = m_geometry.m_d   (kv.ie,idim,jdim,ipt,jpt);
    });
    kv.team_barrier();
  ...
  void compute_div_vdp(KernelVariables &kv) const {
    ...
    // Compute div(vdp)
    m_sphere_ops.divergence_sphere(kv,kv.dvv,kv.Dinv,kv.metdet,
  ...
  bool compute_scan_quantities (KernelVariables &kv) const {
    ...
    // Load now, so you do one barrier only
    auto pi_shared = load_shared_scalar<NUM_LEV>(kv,Homme::subview(m_buffers.pi,kv.team_idx));
    kv.team_barrier();

    // Compute grad(pi)
    m_sphere_ops.gradient_sphere(kv,kv.dvv,kv.Dinv,
                                    pi_shared,
  ...
  void compute_w_and_phi_tens (KernelVariables& kv) const {
    using namespace PhysicalConstants;
    ScratchView<Scalar[NP][NP][NUM_LEV_P]> temp1_i(reinterpret_cast<Scalar*>(kv.team_shmem));
    ScratchView<Scalar[NP][NP][NUM_LEV_P]> temp2_i(temp1_i.data() + NP*NP*NUM_LEV_P);
    ...
    load_shared_scalar<NUM_LEV_P>(kv,Homme::subview(m_state.m_phinh_i,kv.ie,m_data.n0),temp1_i);
    load_shared_scalar<NUM_LEV_P>(kv,Homme::subview(m_state.m_w_i,kv.ie,m_data.n0),temp2_i);
    kv.team_barrier();
    m_sphere_ops.gradient_sphere(kv,kv.dvv,kv.Dinv,temp1_i,grad_phinh_i);
    m_sphere_ops.gradient_sphere(kv,kv.dvv,kv.Dinv,temp2_i,grad_w_i);
    kv.team_barrier();
  ...
  void compute_dp_and_theta_tens (KernelVariables &kv) const {
    ...
    ScratchView<Scalar[NP][NP][NUM_LEV]> vtheta;
    if (m_theta_advection_form==AdvectionForm::Conservative) {
      ...
      if (m_rsplit==0) {
        ...
        m_sphere_ops.divergence_sphere_cm<CM::Add>(kv,kv.dvv,kv.Dinv,kv.metdet,v_vtheta_dp,
                                          Homme::subview(m_buffers.theta_tens,kv.team_idx));
      } else {
        m_sphere_ops.divergence_sphere(kv,kv.dvv,kv.Dinv,kv.metdet,v_vtheta_dp,
                                          Homme::subview(m_buffers.theta_tens,kv.team_idx));
      }
    } else {
      auto vtheta_lambda = [&](const int igp,const int jgp,const int ilev)->Scalar {
        return vtheta_dp(igp,jgp,ilev) / dp(igp,jgp,ilev);
      };
      vtheta = load_shared_scalar<NUM_LEV>(kv,vtheta_lambda);
      kv.team_barrier();
      m_sphere_ops.gradient_sphere(kv,kv.dvv,kv.Dinv,vtheta,
                                      Homme::subview(m_buffers.grad_tmp,kv.team_idx));
    }
    kv.team_barrier();
  ...
  void compute_v_tens (KernelVariables &kv) const {
    // Used for gradient_sphere calls (we never use both at the same time, so they can alias each other)
    ScratchView<Scalar[NP][NP][NUM_LEV]>   temp(reinterpret_cast<Scalar*>(kv.team_shmem));
    ScratchView<Scalar[NP][NP][NUM_LEV_P]> temp_i(reinterpret_cast<Scalar*>(kv.team_shmem));
    ...
    // Compute vorticity(v)
    m_sphere_ops.vorticity_sphere(kv, Homme::subview(m_state.m_v,kv.ie,m_data.n0), vort);

    if (m_theta_hydrostatic_mode) {
      // In nh mode, gradphinh has already been computed, but in hydro mode
      // we skip the whole compute_w_and_phi_tens call
      load_shared_scalar<NUM_LEV_P>(kv,Homme::subview(m_state.m_phinh_i,kv.ie,m_data.n0),temp_i);
      kv.team_barrier();
      m_sphere_ops.gradient_sphere(kv,kv.dvv,kv.Dinv,temp_i,
      ...
      Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team,NP*NP),
        ...
        ColumnOps::compute_midpoint_values<CombineMode::Scale>(kv, w_sq,Homme::subview(temp,igp,jgp),0.5);
      });
      kv.team_barrier();

      // Compute grad(average(w^2/2)). Store in wvor.
      m_sphere_ops.gradient_sphere(kv, kv.dvv, kv.Dinv, temp, wvor);

      // Compute grad(w)
      load_shared_scalar<NUM_LEV_P>(kv.team,Homme::subview(m_state.m_w_i,kv.ie,m_data.n0),temp_i);
      kv.team_barrier();
      m_sphere_ops.gradient_sphere(kv, kv.dvv, kv.Dinv, temp_i,
    ...
    load_shared_scalar<NUM_LEV>(kv.team,Homme::subview(m_buffers.exner,kv.team_idx),temp);
    kv.team_barrier();
    m_sphere_ops.gradient_sphere(kv, kv.dvv, kv.Dinv, temp, grad_exner);
    ...
    if (m_pgrad_correction) {
      ...
      load_shared_scalar<NUM_LEV>(kv.team,log_exner,temp);
      kv.team_barrier();
      m_sphere_ops.gradient_sphere(kv, kv.dvv, kv.Dinv, temp, grad_tmp);
    }
    kv.team_barrier();
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(kv.team,NP*NP),
      ...
      if (m_pgrad_correction) {
        ...
        Kokkos::parallel_for(Kokkos::ThreadVectorRange(kv.team,NUM_LEV),
                             [&](const int ilev) {
          auto tmp = grad_exner_i_x(ilev);
          tmp /= exner_i(ilev);
          tmp += grad_tmp_i_x(ilev);
          tmp *= cp*T0;
          mgrad_x(ilev) += tmp;

          tmp = grad_exner_i_y(ilev);
          tmp /= exner_i(ilev);
          tmp += grad_tmp_i_y(ilev);
          tmp *= cp*T0;
          mgrad_y(ilev) += tmp;
      ...
      auto u  = Homme::subview(m_state.m_v,kv.ie,m_data.n0,0,igp,jgp);
      auto v  = Homme::subview(m_state.m_v,kv.ie,m_data.n0,1,igp,jgp);
      auto KE = Homme::subview(temp,igp,jgp);
    ...
    if (m_rsplit==0) {
      ...
      m_sphere_ops.gradient_sphere_update(kv, kv.dvv, kv.Dinv,
                                              temp,
