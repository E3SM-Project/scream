...
{
  const std::array< std::pair<Real, Real>, MomUbycondData::NUM_ARRAYS > ranges = {
    std::make_pair(-1.0, 1.0), // thl
    std::make_pair(-1.0, 1.0), // qw
    std::make_pair(-1.0, 1.0), // qwthl
    std::make_pair(-1.0, 1.0), // wthl
    std::make_pair(-1.0, 1.0), // uw
    std::make_pair(-1.0, 1.0), // vw
    std::make_pair(-1.0, 1.0), // wtke
    std::make_pair(-1.0, 1.0), // wtracer
  };

  MomUbycondData uby_fortran[] = {
    // shcol, num_tracer, ranges
    MomUbycondData(128, 20, ranges),
    MomUbycondData(128, 20, ranges),
    MomUbycondData(128, 20, ranges),
    MomUbycondData(128, 20, ranges),
  };

  static constexpr Int num_runs = sizeof(uby_fortran) / sizeof(MomUbycondData);

  // Create copies of data for use by cxx. Needs to happen before fortran calls so that
  // inout data is in original state
  MomUbycondData uby_cxx[num_runs] = {
    MomUbycondData(uby_fortran[0]),
    MomUbycondData(uby_fortran[1]),
    MomUbycondData(uby_fortran[2]),
    MomUbycondData(uby_fortran[3]),
  };
