...
555   continue
 ...
 subroutine p3_main_part3(kts, kte, kbot, ktop, kdir, &
   ...
   real(rtype)    :: qc_incld                  ! cloud-averaged qc specific humidity  
   real(rtype)    :: nc_incld                  ! cloud-averaged nc drop concentration
   real(rtype)    :: qr_incld                  ! rainy-area-averaged qr specific humidity  
   real(rtype)    :: nr_incld                  ! rainy-area-averaged nr drop concentration
   ...
   k_loop_final_diagnostics:  do k = kbot,ktop,kdir
      ...
      if (qr(k).ge.qsmall) then
         ...
         !PMC BUG: CHANGED NR TO NR_INCLD BELOW, BUT REFLECTIVITY IS PROBABLY AN IN-CLOUD PROPERTY NOW.
         !SHOULD IT BE TURNED INTO A CELL-AVE THING?
         ze_rain(k) = nr_incld*(mu_r(k)+6._rtype)*(mu_r(k)+5._rtype)*(mu_r(k)+4._rtype)*           &
      ...
      qi_not_small:  if (qi(k).ge.qsmall) then
         ...
         !PMC BUG: ALL(?) THE CALCULATIONS IN THE QI SECTION SHOULD USE IN-CLOUD VALUES.
         !NOT FIXING NOW BECAUSE NEEDS cld_frac_i TO BE PASSED INTO part3 TO IMPLEMENT.
...
333    continue
  ...
  subroutine check_temp(t, subname)
    !Check if temperature values are in legit range
    use scream_abortutils, only : endscreamrun
