...
Real calculate_water_mass(const std::shared_ptr<const GridsManager>& grids_mgr,const std::shared_ptr<FieldManager>& field_mgr, const Real dt, const bool use_precip) {
  ...
  const auto& grid = grids_mgr->get_grid("Physics GLL");
  ...
  if (use_precip) {
    auto d_tmp_liq = field_mgr->get_field("precip_liq_surf_mass").get_view<Real*>();
    auto d_tmp_ice = field_mgr->get_field("precip_ice_surf_mass").get_view<Real*>();
    Real result;
    Kokkos::parallel_reduce("",ncol, KOKKOS_LAMBDA(const int& icol,Real& lsum) {
      lsum += d_tmp_liq(icol) + d_tmp_ice(icol);
    },result);
    total_mass += result;
