...
void SurfaceCouplingExporter::set_grids(const std::shared_ptr<const GridsManager> grids_manager)
{
  ...
  create_internal_field<Real>("Sa_z",       scalar2d_layout, grid_name);
  create_internal_field<Real>("Sa_u",       scalar2d_layout, grid_name);
  create_internal_field<Real>("Sa_v",       scalar2d_layout, grid_name);
  create_internal_field<Real>("Sa_tbot",    scalar2d_layout, grid_name);
  create_internal_field<Real>("Sa_ptem",    scalar2d_layout, grid_name);
  create_internal_field<Real>("Sa_pbot",    scalar2d_layout, grid_name);
  create_internal_field<Real>("Sa_shum",    scalar2d_layout, grid_name);
  create_internal_field<Real>("Sa_dens",    scalar2d_layout, grid_name);
  create_internal_field<Real>("Sa_pslv",    scalar2d_layout, grid_name);
  create_internal_field<Real>("Faxa_rainl", scalar2d_layout, grid_name);
  create_internal_field<Real>("Faxa_snowl", scalar2d_layout, grid_name);
  create_internal_field<Real>("Faxa_swndr", scalar2d_layout, grid_name);
  create_internal_field<Real>("Faxa_swvdr", scalar2d_layout, grid_name);
  create_internal_field<Real>("Faxa_swndf", scalar2d_layout, grid_name);
  create_internal_field<Real>("Faxa_swvdf", scalar2d_layout, grid_name);
  create_internal_field<Real>("Faxa_swnet", scalar2d_layout, grid_name);
  create_internal_field<Real>("Faxa_lwdn",  scalar2d_layout, grid_name);
}
// =========================================================================================
...
void SurfaceCouplingExporter::initialize_impl (const RunType /* run_type */)
{
  ...
  for (int i=0; i<m_num_scream_exports; ++i) {
    ...
    EKAT_REQUIRE_MSG(has_internal_field(fname),"Error! Attempting to export "+fname+
                   " which has not been added as an internal field.\n");
    auto& field = get_internal_field(fname);
  ...
  if (m_params.isSublist("prescribed_from_file")) {
    ...
    if (do_export_from_file) {
      ...
      for (auto fname : export_from_file_fields) {
        ...
        auto& f_internal = get_internal_field(fname);
        // We want to add the field as a deep copy so that the internal_fields are automatically updated.
        time_interp.add_field(f_internal, true);
...
void SurfaceCouplingExporter::set_constant_exports()
{
  ...
  for (int i=0; i<m_num_scream_exports; ++i) {
    if (m_export_source_h(i)==CONSTANT) {
      std::string fname = m_export_field_names[i];
      const auto field_view = get_internal_field(fname).get_view<Real*>();
...
void SurfaceCouplingExporter::compute_eamxx_exports(const double dt, const bool called_during_initialization)
{
  ...
  const auto Sa_z       = get_internal_field("Sa_z").get_view<Real*>();
  const auto Sa_u       = get_internal_field("Sa_u").get_view<Real*>();
  const auto Sa_v       = get_internal_field("Sa_v").get_view<Real*>();
  const auto Sa_tbot    = get_internal_field("Sa_tbot").get_view<Real*>();
  const auto Sa_ptem    = get_internal_field("Sa_ptem").get_view<Real*>();
  const auto Sa_pbot    = get_internal_field("Sa_pbot").get_view<Real*>();
  const auto Sa_shum    = get_internal_field("Sa_shum").get_view<Real*>();
  const auto Sa_dens    = get_internal_field("Sa_dens").get_view<Real*>();
  const auto Sa_pslv    = get_internal_field("Sa_pslv").get_view<Real*>();
  const auto Faxa_rainl = get_internal_field("Faxa_rainl").get_view<Real*>();
  const auto Faxa_snowl = get_internal_field("Faxa_snowl").get_view<Real*>();
  const auto Faxa_swndr = get_internal_field("Faxa_swndr").get_view<Real*>();
  const auto Faxa_swvdr = get_internal_field("Faxa_swvdr").get_view<Real*>();
  const auto Faxa_swndf = get_internal_field("Faxa_swndf").get_view<Real*>();
  const auto Faxa_swvdf = get_internal_field("Faxa_swvdf").get_view<Real*>();
  const auto Faxa_swnet = get_internal_field("Faxa_swnet").get_view<Real*>();
  const auto Faxa_lwdn  = get_internal_field("Faxa_lwdn" ).get_view<Real*>();
  ...
  Kokkos::parallel_for(setup_policy, KOKKOS_LAMBDA(const Kokkos::TeamPolicy<KT::ExeSpace>::member_type& team) {
    ...
    // Set the values in the internal fields which correspond to the exported variables
