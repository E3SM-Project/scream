...
void RRTMGPRadiation::set_grids(const std::shared_ptr<const GridsManager> grids_manager) {
  ...
  add_field<Required>("qc", scalar3d_mid, kgkg, grid_name, ps);
  add_field<Required>("nc", scalar3d_mid, 1/kg, grid_name, ps);
  add_field<Required>("qi", scalar3d_mid, kgkg, grid_name, ps);
  add_field<Required>("cldfrac_tot", scalar3d_mid, nondim, grid_name, ps);
  add_field<Required>("eff_radius_qc", scalar3d_mid, micron, grid_name, ps);
  add_field<Required>("eff_radius_qi", scalar3d_mid, micron, grid_name, ps);
  add_field<Required>("qv",scalar3d_mid,kgkg,grid_name, ps);
  ...
  for (auto& it : m_gas_names) {
    ...
    if (it == "o3") {
      ...
      add_field<Updated >(it + "_volume_mix_ratio", scalar3d_mid, molmol, grid_name, ps);
    } else {
      // the rest are computed from prescribed surface values
      add_field<Computed>(it + "_volume_mix_ratio", scalar3d_mid, molmol, grid_name, ps);
    }
  }
  // Required aerosol optical properties from SPA
  m_do_aerosol_rad = m_params.get<bool>("do_aerosol_rad",true);
  if (m_do_aerosol_rad) {
    add_field<Required>("aero_tau_sw", scalar3d_swband, nondim, grid_name, ps);
    add_field<Required>("aero_ssa_sw", scalar3d_swband, nondim, grid_name, ps);
    add_field<Required>("aero_g_sw"  , scalar3d_swband, nondim, grid_name, ps);
    add_field<Required>("aero_tau_lw", scalar3d_lwband, nondim, grid_name, ps);
  }

  // Set computed (output) fields
  add_field<Updated >("T_mid"     ,            scalar3d_mid, K  ,    grid_name, ps);
  add_field<Computed>("SW_flux_dn",            scalar3d_int, Wm2,    grid_name, "RESTART", ps);
  add_field<Computed>("SW_flux_up",            scalar3d_int, Wm2,    grid_name, "RESTART", ps);
  add_field<Computed>("SW_flux_dn_dir",        scalar3d_int, Wm2,    grid_name, ps);
  add_field<Computed>("LW_flux_up",            scalar3d_int, Wm2,    grid_name, "RESTART", ps);
  add_field<Computed>("LW_flux_dn",            scalar3d_int, Wm2,    grid_name, "RESTART", ps);
  add_field<Computed>("SW_clrsky_flux_dn",     scalar3d_int, Wm2,    grid_name, ps);
  add_field<Computed>("SW_clrsky_flux_up",     scalar3d_int, Wm2,    grid_name, ps);
  add_field<Computed>("SW_clrsky_flux_dn_dir", scalar3d_int, Wm2,    grid_name, ps);
  add_field<Computed>("LW_clrsky_flux_up",     scalar3d_int, Wm2,    grid_name, ps);
  add_field<Computed>("LW_clrsky_flux_dn",     scalar3d_int, Wm2,    grid_name, ps);
  add_field<Computed>("rad_heating_pdel",      scalar3d_mid, Pa*K/s, grid_name, "RESTART", ps);
  // Cloud properties added as computed fields for diagnostic purposes
  add_field<Computed>("cldlow"        , scalar2d, nondim, grid_name, "RESTART");
  add_field<Computed>("cldmed"        , scalar2d, nondim, grid_name, "RESTART");
  add_field<Computed>("cldhgh"        , scalar2d, nondim, grid_name, "RESTART");
  add_field<Computed>("cldtot"        , scalar2d, nondim, grid_name, "RESTART");
  // 0.67 micron and 10.5 micron optical depth (needed for COSP)
  add_field<Computed>("dtau067"       , scalar3d_mid, nondim, grid_name, "RESTART");
  add_field<Computed>("dtau105"       , scalar3d_mid, nondim, grid_name, "RESTART");
  add_field<Computed>("sunlit"        , scalar2d    , nondim, grid_name, "RESTART");
  // Cloud-top diagnostics following AeroCOM recommendation
  add_field<Computed>("T_mid_at_cldtop", scalar2d, K, grid_name, "RESTART");
  add_field<Computed>("p_mid_at_cldtop", scalar2d, Pa, grid_name, "RESTART");
  add_field<Computed>("cldfrac_ice_at_cldtop", scalar2d, nondim, grid_name, "RESTART");
  add_field<Computed>("cldfrac_liq_at_cldtop", scalar2d, nondim, grid_name, "RESTART");
  add_field<Computed>("cldfrac_tot_at_cldtop", scalar2d, nondim, grid_name, "RESTART");
  add_field<Computed>("cdnc_at_cldtop", scalar2d, 1 / (m * m * m), grid_name, "RESTART");
  add_field<Computed>("eff_radius_qc_at_cldtop", scalar2d, micron, grid_name, "RESTART");
  add_field<Computed>("eff_radius_qi_at_cldtop", scalar2d, micron, grid_name, "RESTART");
  ...
  add_field<Computed>("sfc_flux_dir_nir", scalar2d, Wm2, grid_name, "RESTART");
  add_field<Computed>("sfc_flux_dir_vis", scalar2d, Wm2, grid_name, "RESTART");
  add_field<Computed>("sfc_flux_dif_nir", scalar2d, Wm2, grid_name, "RESTART");
  add_field<Computed>("sfc_flux_dif_vis", scalar2d, Wm2, grid_name, "RESTART");
  add_field<Computed>("sfc_flux_sw_net" , scalar2d, Wm2, grid_name, "RESTART");
  add_field<Computed>("sfc_flux_lw_dn"  , scalar2d, Wm2, grid_name, "RESTART");
