...
if [ $skip_testing -eq 0 ]; then
  ...
  ./scripts/gather-all-data "module list" -l -m $SCREAM_MACHINE # && ./scripts/test-all-scream --baseline-dir $BASELINES_DIR \$compiler -c EKAT_DISABLE_TPL_WARNINGS=ON $AUTOTESTER_CMAKE -p -i -m \$machine $SUBMIT" -l -m $SCREAM_MACHINE
